...
def q3():
    norm = [dataframe['normal'].mean(), dataframe['normal'].var()]
    binom = [dataframe['binomial'].mean(), dataframe['binomial'].var()]
    return (round(binom[0]-norm[0], 3), round(binom[1]-norm[1], 3))
...
stars_mean = stars[stars['target'] == 0]['mean_profile']
false_pulsar_mean_profile_standardized = (stars_mean-stars_mean.mean())/stars_mean.std()
...
def q5():
    quartis = np.percentile(false_pulsar_mean_profile_standardized, [25, 50, 75])
    
    q1 = sct.norm.ppf(0.25, loc=0, scale=1)
    q2 = sct.norm.ppf(0.50, loc=0, scale=1)
    q3 = sct.norm.ppf(0.75, loc=0, scale=1)
    
    return (round(quartis[0]-q1, 3), round(quartis[1]-q2, 3), round(quartis[2]-q3, 3))
